...
async fn run_scan(mut args: ScanArgs, verbose: bool) -> Result<i32, error::AppError> {
    let registry = CheckRegistry::new();
    ...
    if args.socket.is_some() && !args.hosts.is_empty() {
        return Err(error::AppError::Config(
            "-s/--socket cannot be combined with -H/--host".to_string(),
        ));
    }

    // Fall back to PGHOST when no explicit target was given
    if args.hosts.is_empty()
        && args.socket.is_none()
        && let Ok(pghost) = std::env::var("PGHOST")
    {
        let pghost = pghost.trim().to_string();
        if !pghost.is_empty() {
            args.hosts.push(pghost);
        }
    }
    ...
    if !args.has_target() {
        ...
        return Err(error::AppError::Config(
            "No connection target specified. Use -H, -s, PGHOST, or --offline for file-based checks only.".to_string(),
        ));
    }
    ...
    if let Some(ref socket) = args.socket {
        ...
        let client = match connection::connect(&params, verbose).await {
            ...
            Err(e) => {
                if args.offline {
                    eprintln!("{}: {} (continuing in offline mode)", "Warning".yellow(), e);
                    None
                } else {
                    return Err(e.into());
                }
    ...
    let targets = target::resolve_all_targets(&args.hosts, args.allow_large)
    ...
    let config_ref = &scan_config;
    let multi = targets.len() > 1;

    let host_reports: Vec<HostReport> = futures::stream::iter(targets.iter())
        .map(move |target| async move {
            if verbose || multi {
                eprintln!("Scanning {}...", target);
            }
            scan_single_target(target, args_ref, registry_ref, config_ref, verbose).await
        })
...
async fn run_checks_on_host(
    ...
    for check in registry.checks() {
        ...
        if check.requires_connection() && client.is_none() {
            if verbose {
                eprintln!("Skipping {} (requires connection)", check.id());
            }
            continue;
        }
        ...
        match check.execute(client, scan_config).await {
            ...
            Err(e) => {
                if verbose {
                    eprintln!("{}: Check {} failed: {}", "Warning".yellow(), check.id(), e);
                }
            }
        }
    }
